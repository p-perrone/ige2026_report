...
\begin{figure}
	...
	\subfloat[On the left, data at D17 station at Cap Prud'homme; on the right, at EAIIST02 station in the megadunes zone]{\includegraphics[width=17cm]{stations_mb_sice}}	
	\mycaption{\emph{In situ} and satellite retrieved SSAs}{Comparison between Multiband and S-3/OLCI}
	\label{fig:tsex}
	\vspace{-0.3cm}
\end{figure}

Time series of snow SSA were successfully obtained for all five stations from both Multiband records and S-3/OLCI data. A representative example for the two locations is shown in Figure~\ref{fig:tsex}, with full stacks reported in Appendix~\ref{a:timeseries}.

At D17---and Cap Prud'homme in general---Multiband-retrieved SSA values are typically below \qty{20}{\ssaunit} in mid-austral summer, reaching up to \qty{50}{\ssaunit} at the beginning and end of the season. The series are generally continuous, with few interruptions, indicating proper instrument functioning. Multiband records show similar sizes and dynamics across the three stations, reflecting consistent surface environmental conditions. OLCI-retrieved SSA for this location agrees well with Multiband data on average (biases below \qty{6}{\ssaunit}), except at the start of each summer season, when satellite SSA is significantly lower by $\sim 25$--\qty{50}{\ssaunit}. OLCI series are sparser, with NaNs typically occurring when Multiband signals peak abruptly, likely due to precipitation and overcast conditions preventing surface visibility. Overall, \emph{in situ} and satellite observations are consistent across all years and Cap Prud'homme stations.

In the megadunes, Multiband-measured SSA is generally higher, reflecting the absence of wet metamorphism and smaller grain sizes. The signals from the two stations diverge in mid-summer, with stronger year-to-year variability than at Cap Prud'homme. When comparing Multiband records to OLCI, an issue arises: the two methods yield different SSAs. For EAIIST01, the offset is only slightly higher than at coastal stations, but the time series show a more systematic bias. For EAIIST02, the bias is nearly \qty{25}{\ssaunit}. However, at EAIIST01, the two time series still capture the same temporal evolution of snow grain size, with relative changes in SSA occurring in the same direction and magnitude. This is not the case for EAIIST02, where the two methods sometimes register different evolutions, particularly in 2022 and 2023. Quantitatively, the correlation coefficient is $r=0.65$ for EAIIST01 and $r=0.21$ for EAIIST02 (Figure~\ref{fig:corr_sice_mb}). To understand and address these differences, we proceed with the diagnostics described in the Methods.
