...
=======
	...
	Being such a harsh and inaccessible place, it has only allowed \emph{in situ} investigations limited to regional scales and relatively short time coverage. This has led the scientific community to rely on satellite data, which inherits higher uncertainties and reduced spatio-temporal resolution, from missions such as AVHRR, MODIS, and CLARA-A2.1-SAL \parencite{callejaSnowAlbedoSeasonality2019, yuqi}.\\

	
	With this work, we aim to provide a better understanding of the evolution of surface albedo in Antarctica by combining an \emph{in situ} approach with satellite observations. By exploiting data from automated albedometers installed in several location on the AIS, we intend to produce time series of snow locally measured SSA, and compare them with the satellite-retrieved grain size. Such measurement stations were mounted in two locations, mainly: on the coast, near the French station Dumont d'Urville, and in a megadune field, with an albedometer installed on the erosion area an other on the accumulation area of a dune. By performing this analysis, the overarching goals are:
	\begin{itemize}
		\item validating a satellite product, ESA's Sentinel-3 Ocean and Land Color Instrument), which would allow the observation of long-term and continent-wide study of snow albedo at a relatively fine resolution ($\sim\qty{300}{\meter}$), compared to the one of other missions (\eg, MODIS);
		\item deepen the understanding of processes driving the temporal change in albedo at the local scale, particularly on megadunes, thanks to an unprecedented 6-years continuous record of albedo.  
	\end{itemize} 
>>>>>>> 3343925c3621b90a9c1b9d78d9fe9d1b176a04d0
