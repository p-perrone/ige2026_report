...
Regarding Multiband records from Cap Prud'homme, we observe that fluctuations in grain size are well captured both at the short- and long-term time scale. Starting from the seasonal signal, we observe that SSA has the tendency to be lowest in early December to end of January, when summer temperatures are highest. This reflects well the effect of wet metamorphism, as also previously stated by \cite{arioliDynamicsSnowGrain2023} based on \cite{vandecruxDeterminationSnowOptical2022}'s finding that $\ssa = \qty{10.22}{\ssaunit}$ represents a possible threshold for discriminating days of unlikely melt and days of intense snowmelt. In fact, several days of grain size below this critical value are observed at the three stations, particularly around end of December and mid-February. The snowfall dynamics are also well visible in the time series: a typical precipitation yields an abrubt increase in the SSA upt to values typical of fresh fallen snow ($\ssa > \qty{60}{\ssaunit}$, \cite{domineThreeExamplesWhere2009}). These events are usually followed by a more gradual increase in grain size given by the coupled effects of wind and metamorphism, with values between 10 and \qty{40}{\ssaunit}. We can clearly see how these peacks direclty correlata with snowfalls recorded by the SR50 sensor in Figure
