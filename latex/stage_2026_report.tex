...
\begin{figure}
	...
	\label{fig:tsex}
\end{figure}
Time series of snow SSA were successfully obtained for all the five stations and from both Multiband records and S-3/OLCI data. A representative example of the two locations is show in Figure \ref{fig:tsex}; the stacks from all years are reported in the Appendix \ref{a:timeseries}.

At D17---Cap Prud'homme in general---Multiband retrieved SSA values are typically lower than \qty{20}{\ssaunit} in mid-austral summer, whereas they tend to reach up to more than \qty{50}{\ssaunit} at the beginning and at the end; the series are in general continuous with few interrruption in the signal, meaning a proper functioning of the instrument. Multiband records much similar sizes and dynamics across the three stations, meaning consistent environemental conditions at surface in that area. OLCI-retrieved SSA for this location show a relatively good agreement with Multiband data, exception made for each beginning of summer season, when the satellite's SSA is singificantly lower of $\sim 25$--\qty{50}{\ssaunit}. OLCI series are also much sparser and NaNs are typically registered when Multiband signal abruptly peaks---in that situation, precipitation likely occurs and the sky is therefore overcast, preventing the satellite from seeing the surface. Overall, throughout all the years and for all cap Prud'homme stations, there is consistency between \emph{in situ} an satellite observations.

In the megadunes, the SSA measured by Multiband is generally higher, reflecting the absence of wet metamorphism and subsequent smaller grain size. The signals from the two stations are also diverging one from the other in mid-summer and their year-toyear variability is much stronger than at Cap Prud'homme; the reasons of such behaviour will be explain later on in the Discussion. When comparing Multiband records to OLCI, the most relevant issue of this analysis arise: in contrast to Cap Prud'homme, the two methods are yielding significantly different SSAs in terms of absolute values---the offset between the two is typically $\sim 25$--\qty{30}{\ssaunit}, representing non-negligible. However, the two time series are typically able to register the same temporal evolution of snow grain size: this consistent co-dynamic can notably be seen in Figure \ref{fig:tsex}---SSA changes relatively in time of the same amount and in the same direction (positive or negative change) on both Multiband and OLCI record. In fact, quantitatively, the correlation coefficient between the two series is failry good at r and r respectively, whereas the bias is very high (Figure \ref{fig:corr_sice_mb}) With the aim of understanding and fixing this gap, we proceed with diagnostics described in the Methods.
...
When looking at the signals 
 
 		\subsection{OLCI issues}
As we noticed that, for some seasons and for EAIIST02 station, SICE and Multiband are yileding completely different grain size dynamics, , we inspected S-3/OLCI data and its SICE 1.6 processing. Similarly as what we did with Multiband, the sub-daily variations in the signal in albedo from both the EAIIST stations were inspected. After having selected measurements from good weather dates only, we performed the analysis on random clusters of consecutive days, excluding hours with $\mathrm{CSI} \geq 1.0$. We found that the variability is in general fairly strong, with an average sub-daily deviation in the signal of \qty{4.}{\ssaunit} and \qty{3.}{\ssaunit} for EAIIST01 and EAIIST02 respectively. In particular, the type of variability is not unequivuous and is stricly day-dependent: it can be given by a gradual increase in the SSA through the day, as well as by a-directional random fulctuations (Figure \ref{fig:sice_subdaily}), preventing from drawing precise conclusions. In any case, as a monotonous increase in the SSA is often observed and not expected at all---especially clou- and precipitation-free days \parencite{picardSpectralAlbedoMeasurements2020}---we applied the SZA flattening factor to the processing to reduce the effect of this parameter as explained in Section \ref{sssc:sicediagn}, producing two new sets of SSA series with factors $r = 0.3$ and 0.4 (Pr30 and Pr40 respectively). A comparison of the reference processing with these last two is shown in Figure \ref{fig:siceflat}. Although effectively raising the SSA values of $\sim$GAP Pr3040 vs REF------- the amount is not sufficient to fully explain the gap we found between the \emph{in situ} and satellite time series
