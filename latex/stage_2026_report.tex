...
\end{equation}
where $\gamma$ was initially tested at 0.3 and progressively increased to 0.4 in subsequent simulations. By adding this factor, we artificially reduce the dependence of SSA on solar angles, allowing us to evaluate the effectiveness of the algorithm.
...
\end{figure}
The offsets between \emph{in situ} and satellite time series initially led us to consider a potential instrumental issue in the Multiband functioning. We first examined the cross-calibration curves used for the correction at both stations, for each specific wavelength. For EAIIST02, the calibration returned an average $R_\mathrm{CC} = \num{1.055(0.004)}$, confirming consistency throughout the experiment and across all wavelengths---a more than reasonable result that we validated. However, the same analysis for EAIIST01 revealed the first technical issue: the computed ratio is generally very low, particularly at shorter wavelengths. For example, at \qty{500}{\nano\meter} (Figure~\ref{fig:eaiist01cc}), the computed $R_\mathrm{CC}$ is \num{0.660(0.003)} across the experiment. Such a value is unacceptable and is likely a consequence of a poor locking connection between the optical fibers and the other components, as well as potential wear of one of the two cosine collectors.

Concerning the inclinometers, for both EAIIST01 and EAIIST02, excluding some electrical noise at the beginning of each season, the signal yields stable angles fluctuating within a range of $\pm \ang{2.5}$. However, the curves stabilize completely from 2025 onward, indicating a potential interruption in functioning. This measure is therefore not exploitable after that date, and no conclusion can be drawn about the potential tilting of the albedometers' heads.

When examining the sub-daily variability in albedo spectra, we noticed that in many cases, the values do not converge to 1 in the visible range, instead yielding unrealistically high values around 1.2 (<add figure of the spectrum>). <PARTIE PROVISOIRE> We tried to constrain this value in order to weight all the albedo spectra of an offset factor, to bring back the spectra converging at 0.98 in the visible. This was done by computing the average of spectral albedo at \qty{500}{\nano\meter} measured around noon <option A> during the first days following the installation <option B> during a set of good weather days. For coherence, we performed this correction for all the five stations---the correction factors are shown in Table @to do. The albedo-corrected time series are shown in Figure @to do --- <comment the correction --- note, this are methodological statements, but are consequences of a result and I found it hard to motivate them in the Methods section>.
...
\begin{figure}
	...
	\label{fig:solalb}
\end{figure}
We compared SICE 1.6's SSA with grain sizes measured by Solalb during the EAIIST and ICORDA campaigns, introducing a third element of comparison that serves as a reference for precision and reliability, as all measurements are performed manually under optimal conditions. The results are shown in Figure~\ref{fig:solalb}.
