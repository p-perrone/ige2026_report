...
\end{equation} 
These ratios enter the processing chain also during slope correction and SSA estimation. Depending on the specific function, either the direct-to-diffuse or the diffuse-to-total ratio can be given as inputs.
...
\begin{figure}[h]
	...
	\caption{\textbf{Apparent emissivity} | BETTEER in methods}
	\label{fig:appemit}
\end{figure}
\begin{table}[h]
	\centering
	\begin{tabular}{lSSSS}
		...
		EAIIST full summer set fit      & 0.59              & {-}                        & 0     & {-} \\
		\bottomrule
	\end{tabular}
	\mycaption{Coefficients for clear sky emittance model $\epsilon_{AC}$}{}
	\label{tab:kad}
\end{table}
This model can be fitted to obtain the two unknowns ($\epsilon_{AD},\, k$) and derive a reliable expression for $\epsilon_{AC}$ as a function of the location's geography and observed meteorological condition, using a cluster of clear-sky cases. This is done by performing an iteratively reweighted least squares regression over 5 iterations: at each iteration, the model is fitted on the cluster, the residuals are calculated and 25\% of points with the highest residuals are discarded---outliers are therefore efficiently removed and a new, cleaner cluster is being used for the next iteration. Records of air temperature, incoming long-wave radiation, and relative humidity (or dew point temperature) are therefore sufficient inputs. The derivation of such expression has first been performed for Cap Prud'homme using the CNR4 installed at D17. This station is also equipped with a camera framing the surrounding environment, allowing to manually select clear sky days with a high reliability. In Figure \ref{fig:appemit}, the apparent emittance of these observed cloud-free days is plotted as function of the water vapour pressure-to-temperature ratio---fitting on these the model following Equation \eqref{eq:eacfull} yields the coefficients summarized in Table \ref{tab:kad}. By plotting also the apparent emittance of all the days from the summers between 2014 to 2025 at D17, we obtain a bi-modal distribution. The uppermost concentrated part of the scatter represents overcast days, with the measured emittances tapering 1; the lowest part is likely largely represented by clear-sky days, with lower emittances measured at slightly drier conditions (\ie lower $\beta$). By manually separating the two clusters with a hard threshold set at $\epsilon_A = 0.8$, we obtain a second set of clear-sky cases, more empirical this time---fitting the same model on it returns coefficients $\epsilon_{AD} = 0.30$ and $k = 0.70$, comparable to the ones obtained from the observed clear-sky cases. This second set of parameters has then been chosen for the expression of $\epsilon_{AC}$'s linear model at Cap Prud'homme.  The  Due to the unavailability of such data for the megadunes, the CSI for the two EAIIST stations was computed using \href{https://cds.climate.copernicus.eu/datasets/reanalysis-era5-single-levels?tab=overview}{ERA5} reanalysis single levels, with $\lwdn$, \qty{2}{\meter} air temperature, and dew point temperature at an hourly step. A validation of ERA5 performance was carried out using the CNR4 installed at the D17 station (Figure \ref{fig:era5cnr4}). 
...
\end{equation}
When the $\mathrm{CSI}$ exceeds 1, the sky is likely overcast, as the emittance is higher than the clear-sky value. Thus, one can discriminate between clear-sky ($\mathrm{CSI} < 1$) and cloud-covered sky ($\mathrm{CSI} \geq 1$), and define a boolean cloud mask: 1 for overcast skies, 0 for clear skies. For Antarctica, due to the frequent presence of thin cloud layers (\eg cirrus, haze) that exhibit clear-sky-like behaviour, the threshold is set to 1.25 rather than 1 \parencite{arioliDynamicsSnowGrain2023}.

Eventually, a day is classified as clear sky when the mean of all its cloud masks is lower than 0.25, \ie with at least 75\% of measurements cloud-free, and overcast otherwise. Clear-sky days are then assigned the direct-to–diffuse partition previously modeled with SBDART, while overcast days are considered to be fully diffuse for each wavelength.
...
More potentially relevant conclusions can be drawn by exploring how the signal evolves throughout the day at each specific wavelength and at each step of the correction. If the albedometer works correclty, after the dark subtraction we expect the signal to be offset by an amount independent of the wavelength and of the time of the day, particularly the SZA. Secondly, under near-perfeclty direct light conditions, the cross calibrated and cosine corrected signal is expected to be symmetrical with respect to solar noon. Last, the variability of albedo spectra throughout the day can be investigated.
\end{itemize}

		\subsection{Processing of Solalb data}
Solalb records can be processed analogously to Multiband ones. Cross calibration is not performed, since there is just one light collector on the head. A single experiments reports a quality tag (A+, A, B, C) which indicates the consistency of spectra trhoughout the experiment, dependin g on light conditions -- A and A+ records are selected only, with less than 10\% of inetrnal variability between the ten upwards and downwards. Direct-to-diffuse ratio is measured directly with the instrument. Thus consist in measuring first the total incoming radiation with the upward-oriented collector; then, a small metal beam is positioned in front of the collector to remove the direct component of radiation and thus record the diffuse fraction. The two signals---total and diffuse---are saved on the incident and reflected channel respectively. This experiment is usually carried out once a day, and the direct-to-diffuse ratio can be obtained from Equation \eqref{eq:dirdifftot}. For a single day, several Solalb experiments can be executed---either on a transect of few tens of meters or in the same position. The daily SSA is retrieved from the average of each experiment.
