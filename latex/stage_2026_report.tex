...
\begin{figure}
	\centering
	\subfloat[On the left, data at D17 station at Cap Prud'homme; on the right, at EAIIST02 station in the megadunes zone]{\includegraphics[width=17cm]{timeseries_example}}\\
...
In the megadunes, Multiband-measured SSA is generally higher, reflecting the absence of wet metamorphism and smaller grain sizes. The signals from the two stations diverge in mid-summer, with stronger year-to-year variability than at Cap Prud'homme. When comparing Multiband records to OLCI, the most relevant issue arises: the two methods yield significantly different SSAs in absolute values. For EAIIST01, the offset is only slightly higher than at coastal stations, but the time series show a more systematic bias. For EAIIST02, the bias is nearly \qty{25}{\ssaunit}. However, at EAIIST01, the two time series still capture the same temporal evolution of snow grain size, with relative changes in SSA occurring in the same direction and magnitude. This is not the case for EAIIST02, where the two methods sometimes register different evolutions, particularly in 2022 and 2023. Quantitatively, the correlation coefficient is $r=0.65$ for EAIIST01 and $r=0.21$ for EAIIST02 (Figure~\ref{fig:corr_sice_mb}). To understand and address these differences, we proceed with the diagnostics described in the Methods.
...
\begin{figure}
	...
	\subfloat[Cross calibration of EAIIST01 at \qty{500}{\nano\meter}]{\includegraphics[width=cm]{eaiist01_cc_500.pdf}}
...
In the megadunes field, as previously anticipated, Multiband was able to capture different grain size dynamics on the accumulation and erosion zones of a dune. 
